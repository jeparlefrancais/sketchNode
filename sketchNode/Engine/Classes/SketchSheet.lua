...
function class:GetNode(id) --\ReturnType: Node
	--\Doc: Returns the node linked with the given id.
	id = class.Package.Utils.Tests.GetArguments(
...
function class:GetSourceCode(flags) --\ReturnType: string
	...
	flags = class.Package.Utils.Tests.GetArguments(
		{'table', flags} -- The node
	)
	local builder = class.Package.Classes.ScriptBuilder:New()

	-- Start by looking for connected events
	for _, connection in ipairs(self.connections) do
		local node = self:GetNode(connection:GetStartNodeId())
		if node:IsA('EventNode') and connection:GetStartIndex() == 0 then
			local ref = node:GetReference()
			if ref:IsA('ObjectReference') then
				local identifier = builder:AddObjectReference(ref)
				local member = ref:GetMember()
				local variables = {}
				for _, typeVar in pairs(node:GetReturnValues()) do
					table.insert(variables, typeVar:GetName())
				end
				builder:AddEventConnection(
					string.format('%s.%s', identifier, member),
					variables,
					{}
				)
			else
				warn(string.format('Can not connect event with reference class <%s>.', ref:GetClassName()))
			end
		end
	end
